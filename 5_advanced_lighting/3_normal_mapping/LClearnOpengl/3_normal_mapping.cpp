...
//    glfwWindowHint(GL_MULTISAMPLE, 4); // 对默认窗口设置多重采样，所有缓冲增加4倍，使用自定义msaa帧缓冲时取消
    ...
    // --------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "buddy_normal_mapping", NULL, NULL);
    if (window == NULL)
    {
    ...
    // -------------------------
    Shader normalMappingShader("shader/normal_mapping.vs", "shader/normal_mapping.fs");
    ...
    // 加载纹理
    // -------------------------
    unsigned int diffuseTexture = loadTexture("texture/brickwall.jpg");
    unsigned int normalMap = loadTexture("texture/brickwall_normal.jpg");
