...
int main(){
    ...
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Buddy_ligthingMaps", NULL, NULL);
    if (window == NULL)
    {
...
//    unsigned int specularMap = loadTexture("lighting_maps_specular_color.png"); // 彩色高光图
    unsigned int emissionMap = loadTexture("matrix.jpg"); //发亮条
...
//        model = glm::rotate(model, 30.0f, glm::vec3(1.0f, 2.0f, 0.3f));
        ...
        lightShader.setVec3("light.diffuse", 0.5f, 0.5f, 0.5f); //将光照调暗了些
        lightShader.setVec3("light.specular", 1.0f, 1.0f, 1.0f);
        lightShader.setFloat("matrixlight", 0.5 + sin((float)glfwGetTime()) / 2);
        lightShader.setFloat("matrixmove", 0.5f * glfwGetTime());
