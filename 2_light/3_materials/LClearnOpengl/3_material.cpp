...
int main(){
    ...
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Buddy_material", NULL, NULL);
    if (window == NULL)
    {
...
//    glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
    ...
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*) (3 * sizeof(float))); // 法线属性
...
//    glEnableVertexAttribArray(2);
    ...
    lightShader.setVec3("material.specular", 0.297254f,    0.30829f,    0.306678f);
    lightShader.setFloat("material.shininess", 12.8f);
...
//        lightPos.y = sin(glfwGetTime() / 2.0f) * 1.0f;
        ...
        model = glm::rotate(model, 0.1f*(float)glfwGetTime(), glm::vec3(1.0f, 0.5f, 0.3f));
        glm::mat4 view = camera.GetViewMatrix();
        glm::mat4 projection = glm::perspective(glm::radians(camera.Zoom), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);
        
        glm::vec3 lightColor;
        lightColor.x = sin(glfwGetTime() * 2.0f); // *2.0 使red分量变化的更快一些
        ...
        lightShader.setVec3("light.diffuse", 1.0f, 1.0f, 1.0f); //将光照调暗了些
...
//        glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
        ...
        // 渲染立方体
        glBindVertexArray(cubeVAO);
        glDrawArrays(GL_TRIANGLES, 0, 36); // 仅用VBO VAO
