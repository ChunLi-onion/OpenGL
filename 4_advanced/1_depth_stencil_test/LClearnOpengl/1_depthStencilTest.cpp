...
//    glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
    ...
    Shader outlineShader("main_obj.vs", "shaderSingleColor.fs");
    // ======================= 设置顶点数据及缓冲，定义顶点属性 ===================
    // 顶点缓冲
    //立方体 6面 * 2个三角形 * 3顶点
    float vertices[] = {
...
//    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glStencilMask(0xFF); //启用模板缓冲写入，箱子所在片段都更新为1；
        ...
        glStencilMask(0x00);
        glDisable(GL_DEPTH_TEST); //轮廓在最上层，关闭深度测试
        ...
        glBindVertexArray(0);
        glEnable(GL_DEPTH_TEST);
        glStencilMask(0xFF); //开启写入才能清理上一帧模板缓存，默认清理为0；
